...
**Tech Stack:** TypeScript, esbuild (bundle to `main.js`), vitest + happy-dom, js-yaml, vis-timeline 8.x (`Graph2d` from `vis-timeline/standalone`), html-to-image, Obsidian plugin API.

**Verification model:** every task is verified by automated tests, including headless rendering of real Graph2d instances under happy-dom (see Task 7). Genuinely visual checks — theme colours, PNG export, mobile, BRAT install — are collected into `docs/superpowers/plans/visual-verification.md` for a single human pass at the end, rather than interrupting each task.
...
Open `LICENSE` and confirm the copyright line reads `Copyright (c) 2026 Roman Borisov`; correct it if the copied file says otherwise.
...
> **SUPERSEDED IN PART.** Review found the `NumericScale`/`CategoryScale`
> design below does not work: with `timeAxis.step > 1` vis snaps ticks to
> day-of-month and computes in local time, so labels are not round in any
> timezone, and `toInternal` overflows the `Date` range for large values such
> as Unix seconds. Read
> `docs/superpowers/plans/2026-07-25-task4-numeric-axis-correction.md` and
> follow it wherever it differs from this task. `TimeScale`, `MomentLike`, the
> `XScale` interface, and `MS_PER_DAY` are unchanged.
...
- Produces:
  - `interface MomentLike { valueOf(): number }`
  - `interface XScale { readonly overridesLabels: boolean; toInternal(value: unknown): number; formatLabel(value: MomentLike): string; axisHints(): Record<string, unknown>; }`
...
**`formatLabel` takes `MomentLike`, not `Date` — this is not negotiable.** vis calls the label callback with its internal **moment** object, which has no `getTime()` method. Calling `getTime()` there throws `TypeError: d.getTime is not a function` on every redraw, breaking the numeric and category axes entirely. This was verified against the bundled library. `valueOf()` is the accessor moment and `Date` share, so the tests below pass plain `Date`s and production passes moments through the same code.
...
- [ ] **Step 4: Add the headless DOM test harness**

Graph2d can be constructed under happy-dom, but happy-dom has no layout engine — every element measures 0×0, so vis computes a degenerate axis range and emits no labels at all. Stubbing element dimensions fixes that. Both facts were verified experimentally before this plan was written; do not re-derive them.

Create `src/test-dom.ts`:
...
import { normalize } from './normalizer';
import { renderGraph2d } from './renderer';
import { stubLayout } from './test-dom';
...
function axisLabels(el: HTMLElement): string[] {
  return Array.from(el.querySelectorAll('.vis-time-axis .vis-text'))
...
describe('renderGraph2d', () => {
  ...
  it('labels a numeric axis with numbers, not dates', () => {
    // The whole point of numeric mode. vis calls the formatter with a
    // moment; if formatLabel reaches for a Date-only method this throws.
    const el = render(
      'options: { xAxis: numeric, start: 0, end: 100 }\n' +
        'x: [0, 25, 50, 75, 100]\ngroups:\n  - id: a\n    y: [0, 1, 2, 3, 4]'
    );
    const labels = axisLabels(el);
    expect(labels.length).toBeGreaterThan(0);
    for (const label of labels) {
      expect(label, `"${label}" is not a plain number`).toMatch(/^-?\d+(\.\d+)?$/);
    }
  });

  it('labels a category axis with the category names', () => {
    const el = render(
      'options: { xAxis: category }\nitems:\n' +
        '  - { x: Mon, y: 1 }\n  - { x: Tue, y: 2 }\n  - { x: Wed, y: 3 }'
    );
    ...
    expect(labels.length).toBeGreaterThan(0);
    for (const label of labels) {
      expect(['Mon', 'Tue', 'Wed']).toContain(label);
    }
  });

  it('does not install a formatter for the time axis', () => {
    // Time mode must keep vis's own date formatting, which produces
    // labels a bare-number pattern would not match.
    const el = render(
      'options: { start: "2026-01-01", end: "2026-06-01" }\n' +
        'items:\n  - { x: "2026-02-01", y: 1 }\n  - { x: "2026-05-01", y: 2 }'
    );
    expect(axisLabels(el).length).toBeGreaterThan(0);
  });
...
Run: `npx vitest run src/renderer.test.ts`
Expected: FAIL — `Failed to resolve import "./renderer"` (or, if renderer.ts is already written from Steps 1–3, failures naming the missing `test-dom` helper).
...
If the numeric-label test throws `d.getTime is not a function`, `formatLabel` is treating its argument as a `Date`. It receives a moment — use `valueOf()`.

- [ ] **Step 8: Verify the console stays clean**

vis's option validator logs to `console.log` when it rejects an option, and the label formatter is exactly the case that trips it. Add this test to `src/renderer.test.ts`:
...
Run: `npx vitest run src/renderer.test.ts`
Expected: PASS — 9 passed. A failure here means the formatter went through `Graph2d.setOptions` instead of `Core.setOptions`.
...
npm run build
npm test
git add src/renderer.ts src/renderer.test.ts src/test-dom.ts src/main.ts styles.css
git commit -m "feat: render vis-graph2d code blocks with all three axis modes"
```

Expected: build exits 0; full suite passes.

Visual confirmation in a real Obsidian vault is deliberately deferred — every visual check across the whole plan is batched into Task 12's single verification pass.
...
- [ ] **Step 8: Add an end-to-end test that a file-backed block renders**
...
Expected: both exit 0. Vault-level confirmation of live refresh on file save is part of Task 12's batched verification pass.
...
- [ ] **Step 3: Test that settings feed defaults and blocks still win**
...
Expected: exits 0 with no type errors. Confirming the settings tab's appearance in Obsidian is part of Task 12's batched verification pass.
...
Expected: all three exit 0. The renderer tests from Task 7 are the regression check that matters here — the export path only activates inside a `[data-pubobs-render]` container, so normal rendering must be completely unaffected. If any Task 7 test now fails, the export changes leaked into the interactive path.

PNG output itself is confirmed in Task 12's batched verification pass; `html-to-image` needs a real canvas and cannot run under happy-dom.
...
- [ ] **Step 2: Check the stylesheet is self-consistent**
...
Expected: build exits 0, and the grep reports a non-zero count — every default colour must come from an Obsidian CSS variable rather than a hardcoded hex, which is what makes the palette follow the user's theme. Confirm by reading `styles.css` that no rule under `.graph2d-plugin` outside the `docs/themes/` files hardcodes a hex colour.

Visual confirmation in light and dark themes is part of Task 12's batched verification pass.
...
- [ ] **Step 6: Check every theme file targets the plugin's container**

```bash
for f in docs/themes/*.css; do
  echo "$f: $(grep -c 'graph2d-plugin' "$f") selectors"
done
```

Expected: each file reports a non-zero count. A theme rule that does not scope to `.graph2d-plugin` would leak styling into the rest of the user's vault. Applying a theme in Obsidian and seeing it take effect is part of Task 12's batched verification pass.
...
- [ ] **Step 5: Write a test that renders every documented example**

This is the acceptance gate for "all types of graphs" and "all styles". Rather than eyeballing dozens of blocks, extract every `vis-graph2d` block from the docs and render each one headlessly. A broken example then fails CI instead of quietly shipping.
...
const docsDir = join(__dirname, '..', 'docs', 'examples');
...
describe('documented examples', () => {
  ...
  for (const file of files) {
    ...
    it(`${file} contains at least one example`, () => {
      expect(blocks.length).toBeGreaterThan(0);
    });

    blocks.forEach((source, index) => {
      it(`${file} block ${index + 1} renders`, () => {
        const el = document.createElement('div');
        document.body.appendChild(el);
        // Any throw here is a broken documented example.
        const graph = renderGraph2d(el, normalize(parseBlock(source)));
        expect(el.querySelector('.vis-panel')).not.toBeNull();
        graph.destroy();
      });
    });
...
Also apply this to the README once it exists, by adding `README.md` to the file list in Step 7.

- [ ] **Step 5b: Run it and fix what breaks**

Run: `npx vitest run src/examples.test.ts`
Expected: every block renders.

For each failure, fix the plugin rather than weakening the example — unless the example itself is genuinely wrong. Re-run `npm test` after each fix. Do not proceed while any documented block fails.
...
- [ ] **Step 7: Extend the example test to cover the README**

A README example that does not work is the most expensive kind of bug, so hold it to the same standard. In `src/examples.test.ts`, replace the `files`/`docsDir` setup with:
...
and drive the loops from `sources` (`for (const { name, markdown } of sources)`) instead of re-reading files inside the loop. Update the first test to assert `sources.length` is greater than 1.

Run: `npx vitest run src/examples.test.ts`
Expected: every README and example block renders.

- [ ] **Step 8: Write the batched visual verification checklist**

Everything above is machine-verified. A short list of genuinely visual checks remains that no headless test can cover. Write them to `docs/superpowers/plans/visual-verification.md` so they can be worked through in one pass:
...
npm test
npm run lint
git add README.md docs src styles.css
...
- [ ] **Step 7: Confirm the release is BRAT-resolvable**

BRAT reads `manifest.json` from the latest release and downloads the assets beside it. Verify that machine-checkable part:

```bash
gh release view 0.1.0 --json tagName,isDraft,isPrerelease
curl -sL "https://github.com/rborisov/vis-graph2d/releases/latest/download/manifest.json" | head -20
```

Expected: `isDraft` false, `isPrerelease` false, tag `0.1.0` with no leading `v`, and the curl returns the real manifest JSON with `"id": "vis-graph2d"` and `"version": "0.1.0"`. A 404 from the curl means BRAT will fail too.

The end-to-end BRAT install in a real vault is the final item on `docs/superpowers/plans/visual-verification.md` — add it there:

```markdown
- [ ] In a clean vault: install BRAT, run **BRAT: Add a beta plugin**, enter
      `rborisov/vis-graph2d`, and confirm the plugin installs, enables, and
      renders a chart. This is the definitive acceptance test for the project.
```
